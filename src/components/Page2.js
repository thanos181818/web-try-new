...
const Page2 = ({ videoMetadata, anomalies, thumbnail }) => {
  const isFakeStyle = {
    color: videoMetadata.is_fake === 'Fake' ? 'red' : 'green',
    fontWeight: 'bold',
  };

  const buttonStyle = {
    backgroundColor: videoMetadata.is_fake === 'Fake' ? 'red' : 'green',
    color: 'white',
    border: 'none',
    padding: '10px 20px',
    margin: '5px',
  ...
  return (
    <div className="details-page">
      <div className="action-buttons">
        <button
          onClick={() => downloadReport({videoMetadata, anomalies})}
          style={buttonStyle}
     ...
     <div className="video-info">
      <h2>Video Details</h2>
      {thumbnail && (
          <img src={thumbnail} alt="Video thumbnail" className="video-thumbnail" style={{ maxWidth: '200px', marginBottom: '10px' }} />
        )}
      <p><strong>Video Name: </strong> {videoMetadata?.video_name || 'N/A'}</p>
      <p>
        <strong>Is Fake: </strong> 
        <span style={isFakeStyle}>
          {videoMetadata?.is_fake || 'Unknown'}
        </span>
      </p>
      <p><strong>Fake Confidence: </strong> {videoMetadata?.fake_confidence || 'N/A'}</p>
      <p><strong>Source: </strong>{videoMetadata?.source || 'N/A'}</p>
    </div>
